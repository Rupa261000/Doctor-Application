...
const UserHome = () => {
   ...
   const getUserData = async () => {
      try {
         await axios.post('http://localhost:5000/api/users/getuserdata', {}, {
            headers: {
   ...
   const getDoctorData = async () => {
      try {
         const res = await axios.get('http://localhost:5000/api/users/getalldoctorsu', {
            headers: {
               Authorization: "Bearer " + localStorage.getItem('token')
            },
         });
         if (res.data.success) {
            // Filter only approved doctors to show in the list
            const approvedDoctors = res.data.data.filter(doc => doc.status === 'approved');
               ...
               <div className="sidebar">
                  ...
                  <div className="menu">
                     ...
                     {userdata.isdoctor === true ? <></> : <div className={`menu-items ${activeMenuItem === 'applyDoctor' ? 'active' : ''}`} onClick={() => handleMenuItemClick('applyDoctor')}>
                        <MedicationIcon className='icon' /><Link to="#" onClick={() => handleMenuItemClick('applyDoctor')}>Apply doctor</Link>
                     </div>}
