...
function ApplyDoctor({ userId }) {
   ...
   const handleSubmit = async (values) => {
      ...
      const doctorData = {
         ...
         timings: Array.isArray(timings) ? {
            start: timings[0]?.format('HH:mm'),
            end: timings[1]?.format('HH:mm')
         } : null
      };

      try {
         const res = await axios.post('http://localhost:5000/api/users/registerdoc', { doctor: doctorData, userId: userId }, {
            headers: {
