...
const Notification = () => {
   ...
   const handleAllMarkRead = async () => {
      try {
         const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/user/getallnotification`, { userId: user._id }, {
            headers: {
   ...
   const handledeleteAllMark = async () => {
      try {
         const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/user/deleteallnotification`, { userId: user._id }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
         })
